...
export async function seed(knex: Knex): Promise<void> {
  ...
  // STAGES
  // ============================================
  const [redRocksStage] = await knex('stage_stg')
    .insert({
  ...
  const [msgStage] = await knex('stage_stg')
    .insert({
  ...
  const [genericStage] = await knex('stage_stg')
    .insert({
  ...
  const [genericStage2] = await knex('stage_stg')
    .insert({
      name_stg: 'Standard 20x20 Stage',
      width_stg: 20,
      depth_stg: 20,
      height_stg: 3,
      created_by_stg: 1,
  ...
  // PROJECTS
  // ============================================
  const [project1] = await knex('project_prj')
    .insert({
  ...
  const [project2] = await knex('project_prj')
    .insert({
      id_usr_prj: demoUser.id_usr,
  ...
  const [project3] = await knex('project_prj')
    .insert({
  ...
  // STAGE PLOTS
  // ============================================
  const [stagePlot1] = await knex('stage_plot_stp')
    .insert({
